...
const QuizList = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const quizzesResponse = await fetch(`${import.meta.env.VITE_API_BASE_URL}/backend/api/get_quizzes.php`);
        ...
        if (studentEmail) {
          const statsResponse = await fetch(`${import.meta.env.VITE_API_BASE_URL}/backend/api/get_student_stats.php?email=${encodeURIComponent(studentEmail)}`);
          if (statsResponse.ok) {
